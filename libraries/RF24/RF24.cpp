...
{  
  ...
  #ifdef NGxxx
  delay(0);
  //uint8_t b = pgm_read_byte(&child_pipe_enable[0]);
  //write_register(EN_RXADDR,read_register(EN_RXADDR) | _BV(b)); // Enable RX on pipe0
  write_register(EN_RXADDR,read_register(EN_RXADDR) | _BV(pgm_read_byte(&child_pipe_enable[0]))); // Enable RX on pipe0
  #endif

  // ** start bugfix by Sadakane
  //delay(0); // ok to place delay before stopListening() instead
  uint8_t b = pgm_read_byte(&child_pipe_enable[0]);
  write_register(EN_RXADDR,read_register(EN_RXADDR) | _BV(b)); // Enable RX on pipe0
  //write_register(EN_RXADDR,read_register(EN_RXADDR) | _BV(pgm_read_byte(&child_pipe_enable[0]))); // Enable RX on pipe0
  // ** end bugfix

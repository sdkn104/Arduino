...
#else
  if( _size >= _maxSize ) {
    if( _fileName == DebugOut.FileName() ) DebugOut.setToSerial(); // todo: this is necessary for fileCopy use DebugOut
    ...
    if( !SPIFFS.remove(_fileName) ) PRINTERR("Error: fail to remove file");
    _size = 0;
    if( _fileName == DebugOut.FileName() ) DebugOut.setToPrevious();
  }
...
/*namespace EspNow {
  uint8_t reqWakeup[] = ESPNOW_REQ_WAKEUP;
  uint8_t ackWakeup[] = ESPNOW_ACK_WAKEUP;
  uint8_t reqPoll[]   = ESPNOW_REQ_POLL;
  uint8_t ackPoll[]   = ESPNOW_ACK_POLL;
  uint8_t ackData[]   = ESPNOW_ACK_DATA;
};*/

uint8_t macAddrSTA[][6] = {
 {}, // No.0
 {}, // No.1
 {},
 {0x5C,0xCF,0x7F,0x17,0xB0,0x99}, // No.3
 {0x5C,0xCF,0x7F,0x17,0xC0,0xB2}, // No.4
 {0x5C,0xCF,0x7F,0x17,0xB6,0x1F}  
};
...
void cont_recv_cb(uint8_t *macaddr, uint8_t *data, uint8_t len) {
  ...
  if( data[0]==0xFF && data[1]==0xFF && data[2]==0x1 && data[3]==0x2 && len==4 ) { // req wakeup
      ...
      log += " send ack...\r\n";
      ack[2] = 0x0; ack[3] = data[3];
      esp_now_send(NULL, ack, 4); // ack
      //
      jsonConfig.obj()["mode"] = "STA";
      espNowStatus.configUpdated = true;
      jsonConfig.save();// TODO: dont use SPIFFS(yield) in call back
      ESP.restart(); // 
  }
  // store
  if( data[0]==0xFF && data[1]==0xFF && data[2]==0x0 && len==4 ) { // ack packet
    espNowStatus.ack_cb = true;
    for (int i = 0; i < 6; i++) {
      espNowStatus.ackMac[i] = macaddr[i];
    }
    espNowStatus.ackType = data[3];
  } else { // data or req packet
...
void slave_recv_cb(uint8_t *macaddr, uint8_t *data, uint8_t len) {
  ...
  if(data[0] == 0xFF && data[1] == 0xFF && data[2] == 0x1 && data[3] == 0x1 && len == 4 ) { // poll req
      log += " send ack...\r\n";
      ack[3] = data[3];
      esp_now_send(NULL, ack, 4); // ack
      jsonConfig.obj()["actionPoll"] = 1;
      espNowStatus.configUpdated = true;
  } else if( data[0] == 0xFF && data[1] == 0xFF && data[2] == 0x0 && len == 4 ) { // ack packet
  } else if( data[0] != 0xFF || data[1] != 0xFF || len != 4 ) { // data packet // TODO: check not accurate
      log += " send ack...\r\n";
      ack[3] = 0x0; // data ack
      esp_now_send(NULL, ack, 4);
      jsonConfig.obj()["actionData"] = 1;
      espNowStatus.configUpdated = true;
  } else { // unknown req
      log += " send ack...\r\n";
      ack[3] = data[3];
  ...
  if( data[0]==0xFF && data[1]==0xFF && data[2]==0x0 && len==4 ) { // ack packet
    ...
    espNowStatus.ackType = data[3];
  } else if( data[0]!=0xFF || data[1]!=0xFF || len!=4) { // data packet (TODO: cannot check surely)
    espNowStatus.recv_cb = true;
    for (int i = 0; i < 6; i++) {
      espNowStatus.recvMac[i] = macaddr[i];
    }
    for (int i = 0; i < len; i++) {
      espNowStatus.recvData[i] = data[i]; // TODO: if multiple data packet, overwritten...
    }
    espNowStatus.recvDataLen = len;
  } // not store req packet
  espNowStatus.log += log;
}
...
bool sendEspNow(uint8_t *message, int len) {
  ...
  for (int retry = 0; retry < 3 && !success; retry++) {
    ...
    for (int i = 0; i < 30; i++) {
      ...
      if ( espNowStatus.ack_cb == true ) { // TODO: it should wait only the target type of ack
        DebugOut.println(String("received ack type ")+espNowStatus.ackType);
        success = true;
        break;
      }
      delay(100);
